...
class MCP2515():
    ...
    def Init(self, ID):
		...
		# self.WriteByte(RXM0SIDH,0x00)
		# self.WriteByte(RXM0SIDL,0x00)

		# self.WriteByte(RXF0SIDH,0x00)
		...
		self.WriteByte(TXB1DLC,0x40 | DLC_8)    # Set DLC = 3 bytes and RTR bit*/

		# Set the identifier and initial data for receive buffer 0
		self.WriteByte(RXB0SIDH,0x00)  # Set receive buffer 0 standard identifier, to be modified***
		self.WriteByte(RXB0SIDL,0x60)  # Use standard identifier
		self.WriteByte(RXB0CTRL,0x60)  # Only receive valid information with standard identifiers, FIILHIT0=0 means RXB0, using FILHIT0
		self.WriteByte(RXB0DLC,DLC_8)  # Set the length of received data to 8 bytes

		self.WriteByte(RXF0SIDH,0xFF)  # Initialize receive filter 0, to be modified***
		self.WriteByte(RXF0SIDL,0xE0)
		self.WriteByte(RXM0SIDH,0xFF)  # Initialize receive mask 0, to be modified***
		self.WriteByte(RXM0SIDL,0xE0)

		# Set interrupt for receive buffer 0
		self.WriteByte(CANINTF,0x00)  # Clear interrupt flag bit
		self.WriteByte(CANINTE,0x01)  # Enable receive buffer 0 full interrupt

		self.WriteByte(CANCTRL, REQOP_NORMAL|CLKOUT_ENABLED)  # Set normal mode

		dummy = self.ReadByte(CANSTAT)
		if( OPMODE_NORMAL != (dummy and 0xE0)):
			self.WriteByte(CANCTRL, REQOP_NORMAL|CLKOUT_ENABLED)  # Check
			self.WriteByte(CANCTRL, REQOP_NORMAL|CLKOUT_ENABLED)  # Check if entered normal working mode

    def Send(self, CAN_TX_Buf, length1):
        # ar tempdata, j;
        tempdata = self.ReadByte(CAN_RD_STATUS)
        self.WriteByte(TXB0DLC,length1)
        for j in range(0, length1): 
            self.WriteByte(TXB0D0+j,CAN_TX_Buf[j])

        if(tempdata&0x04):  # Check TXREQ flag bit
            time.sleep(0.01)
            self.WriteByte(TXB0CTRL, 0)  # Clear TXREQ flag bit
            while(1):  # Wait for TXREQ to be cleared
                if(self.ReadByte(CAN_RD_STATUS)&0x04 != 1):
                    break
        self.SPI.writebytes([CAN_RTS_TXB0])  # Request to send transmit buffer 0
        ...
        while(1):
            if(self.ReadByte(CANINTF) & 0x01):
                ...
                for i in range(0, len): 
                    # Put the data in the receive buffer into the internal RAM buffer
